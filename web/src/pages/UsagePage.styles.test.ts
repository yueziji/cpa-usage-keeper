...
describe('UsagePage toolbar styles', () => {
  ...
  it('lets API Key Settings content scroll inside the card instead of being clipped', () => {
    ...
    expect(usagePageStyles).toMatch(/\.apiKeySettingsBody\s*\{[\s\S]*?max-height:\s*var\(--settings-list-scroll-height\);/)
    ...
    expect(apiKeySettingsMobileBlock).toMatch(/\.apiKeySettingsBody\s*\{[\s\S]*?max-height:\s*var\(--settings-list-scroll-height\);/)
  ...
  it('keeps Model Pricing Settings list viewport aligned with API Key Settings without shrinking it behind the form', () => {
    ...
    expect(apiKeyBodyBlock).toMatch(/max-height:\s*var\(--settings-list-scroll-height\);/)
    expect(apiKeySettingsMobileBlock).toMatch(/\.apiKeySettingsBody\s*\{[\s\S]*?max-height:\s*var\(--settings-list-scroll-height\);/)
    expect(pricingGridBlock).toMatch(/max-height:\s*var\(--settings-list-scroll-height\);/)
